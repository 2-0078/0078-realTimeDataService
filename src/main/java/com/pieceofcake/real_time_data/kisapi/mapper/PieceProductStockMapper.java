...
public class PieceProductStockMapper {
    ...
    private final Map<String, AtomicInteger> productSubscriberCount = new ConcurrentHashMap<>();
    ...
    public String getOrAssignStockCode(String productUuid) {
        ...
        // 구독자 수 증가
        productSubscriberCount.computeIfAbsent(productUuid, k -> new AtomicInteger(0)).incrementAndGet();
        log.info("✅ 구독자 증가 - {} 구독자 수: {}", productUuid, productSubscriberCount.get(productUuid).get());
        return stockCode;
    }

    public void removeSubscriberForPieceUuid(String pieceUuid) {
        AtomicInteger count = productSubscriberCount.get(pieceUuid);
        if (count != null) {
            int remaining = count.decrementAndGet();
            log.info("🛑 구독자 감소 - {} 남은 구독자 수: {}", pieceUuid, remaining);

            if (remaining <= 0) {
                String stockCode = productToStockMap.remove(pieceUuid);
                productSubscriberCount.remove(pieceUuid);
                ...
                if (stockCode != null) {
                    ...
                    if (pieceUuids != null) {
                        ...
                        if (pieceUuids.isEmpty()) {
                            ...
                            log.info("🗑 stockCode에 더 이상 매핑된 productUuid 없어서 stockToProductMap 엔트리 제거 - stockCode: {}", stockCode);
                        }
                    }
                }

                log.info("🗑 구독자 0명, 매핑 제거 완료 - pieceUuid: {}", pieceUuid);
            }
        } else {
